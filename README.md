...
2. `+ ADD NEW PROFILE` をクリックし、ホスト名、ポート、ユーザー名、認証情報 (パスワードまたは秘密鍵)、TOSS 送信先リモートディレクトリ、CATCH 受信元リモートディレクトリ (根本パス)、CATCH 保存先ローカルディレクトリを入力して `SAVE` を押す
...
2. プロファイルで指定した CATCH 根本ディレクトリ内のファイル一覧が幾何学カードとして表示される
3. 目的のファイルの `CATCH` ボタンをクリックすると、設定されたローカルディレクトリへダウンロードされる
4. フォルダカードをクリックするとサブディレクトリへ移動できる。上部の `[ UP ]` で 1 つ上の親階層へ戻り、`[ ROOT ]` でいつでもプロファイル指定の根本ディレクトリへ即座に戻ることができる
